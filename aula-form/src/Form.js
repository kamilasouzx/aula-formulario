...
export function Form (){
  ...
  const [dados, setDados] = useState({mensagem:"..."});
    ...
    return (
        <div>
        <h1 className='text-3xl text-gray-900 mb-4'>Cadastro de usuários</h1>
        <form className ='space-y-2' onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <button className="w-1/2 mt-2 p-3 text-white rounded-lg bg-indigo-700 hover:bg-green-600" type='submit'>Enviar</button>
          </div>
        </form>
        </div>
    )
